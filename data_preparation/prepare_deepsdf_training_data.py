...
if __name__ == "__main__":
  ...
  for fname in os.listdir(args.src):
    if fname.endswith(".ply"):
      path_to_file = os.path.join(args.src, fname)
      
      swl_points = o3d.io.read_point_cloud(path_to_file)
      # estimate normals if they don't exist
      if not swl_points.has_normals():
          swl_points.estimate_normals(search_param=o3d.geometry.KDTreeSearchParamKNN(knn=30))
          # Align normals consistently over the surface without a fixed single direction
          swl_points.orient_normals_consistent_tangent_plane(50)
          
          # Center the point cloud to ensure outward facing algorithm works correctly
          # then orient them towards outside (e.g. from center of mass)
          center = swl_points.get_center()
          # Move center out of the bounding box
          camera_location = np.array([0.0, 0.0, 0.0])
          
          # Or simpler: Just orient all outwards from the center of mass
          # by reversing normals that point inwards towards the center
          pts = np.asarray(swl_points.points)
          nrms = np.asarray(swl_points.normals)
          for i in range(len(pts)):
              direction_from_center = pts[i] - center
              if np.dot(nrms[i], direction_from_center) < 0:
                  nrms[i] = -nrms[i]
          swl_points.normals = o3d.utility.Vector3dVector(nrms)
      ...
      #%% Save
      out_dir = os.path.join(args.src, '../SdfSamples', 'Strawberry', fname[:-4], 'laser')
      os.makedirs(out_dir, exist_ok=True)
      output_filename = os.path.join(out_dir, 'samples.npz')
      print('Save to %s ...' % output_filename, flush=True)
      np.savez(output_filename, pos=pos, neg=neg)
